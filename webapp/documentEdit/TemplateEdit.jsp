...
<%
  ...
  String mEditType="2,1";						//第一位可以为0,1,2,3 其中:0不可编辑;1可以编辑,无痕迹;2可以编辑,有痕迹,不能修订;3可以编辑,有痕迹,能修订；
    ...
    if(mRevisionsCount>0){
      ...
      for(var i=1;i<=mRevisionsCount; i++){
        try{
          ...
          //格式化时间
          var modidate=modifydate.getFullYear()+"-"+(parseInt(modifydate.getMonth())+1)+"-"+modifydate.getDate()+" "+modifydate.getHours()+":"+modifydate.getMinutes();//+":"+modifydate.getSeconds()+"0";
          //组合要插入的内容
			 var mString="";
          ...
          mOfficeObj.Application.Selection.TypeText(" ");
			...
			if(username == "" || username==mAuthor){
		      ...
		      //插入组合的痕迹内容
				...
				selRange.Font.Size=10;         //设置字体大小,4号
...
function ShowFinalStatus(){
	webform.WebOffice.WebObject.Application.ActiveWindow.View.ShowRevisionsAndComments = true;
	webform.WebOffice.WebObject.Application.ActiveWindow.View.RevisionsView = 0;
}

//最终状态
function FinalStatus(){
	webform.WebOffice.WebObject.Application.ActiveWindow.View.ShowRevisionsAndComments = false;
	webform.WebOffice.WebObject.Application.ActiveWindow.View.RevisionsView = 0;
}
...
//   webform.WebOffice.ShowWindow = true;                  //控制显示打开或保存文档的进度窗口，默认不显示
  ...
  webform.WebOffice.AppendTools("39","接受所有修订",0);
  webform.WebOffice.AppendTools("13","-",0);
  webform.WebOffice.AppendTools("37","显示修订标记",0);
  webform.WebOffice.AppendTools("13","-",0);
  webform.WebOffice.AppendTools("38","显示最终稿",0);
  webform.WebOffice.AppendTools("13","-",0);
  ...
  webform.WebOffice.AppendTools("13","-",0);
  webform.WebOffice.AppendTools("36","隐藏痕迹",11);
  webform.WebOffice.AppendTools("13","-",0);
  webform.WebOffice.AppendTools("35","显示痕迹",12);
  webform.WebOffice.AppendTools("13","-",0);
...
<!-- 		     <input type=button class=SideButton value="隐藏痕迹" onclick='delteHistroy("",true)'> -->
<!-- 		     <input type=button class=SideButton value="显示痕迹" onclick='newShowHistory("")'> -->
