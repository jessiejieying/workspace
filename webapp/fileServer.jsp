      ...
      try {
        if (DbaObj.OpenConnection()) {
          try {
            ResultSet result = DbaObj.ExecuteQuery(Sql);
            if (result.next()) {
              Sql = "update Document_File set RecordID=?,FileName=?,FileType=?,FileSize=?,FileBody=?,FilePath=?,UserName=?,Descript=? WHERE RecordID='" + mRecordID + "'";
              tag = 1;
            }
            else {
              Sql = "insert into Document_File (FileID,RecordID,FileName,FileType,FileSize,FileBody,FilePath,UserName,Descript) values (?,?,?,?,?,?,?,?,? )";
            }
            result.close();
          }
          catch (SQLException e) {
          ...
          try {
            ...
            if (tag == 1) {
            	...
            	String ranFileId = String.valueOf(System.currentTimeMillis());
                prestmt.setString(1, ranFileId);
                prestmt.setString(2, mRecordID);
                prestmt.setString(3, mFileName);
                prestmt.setString(4, mFileType);
                prestmt.setInt(5, mFileSize);
             //   prestmt.setString(6, "to_date('"+mFileDate+ "','yyyy-mm-dd hh24:mi:ss')");
      ...
      try {
        if (DbaObj.OpenConnection()) {
          try {
            ResultSet result = DbaObj.ExecuteQuery(Sql);
            if (result.next()) {
              Sql = "update Template_File set RecordID=?,FileName=?,FileType=?,FileSize=?,FileBody=?,FilePath=?,UserName=?,Descript=? WHERE RecordID='" + mTemplate + "'";
              tag = 1;
            }
            else {
              Sql = "insert into Template_File (TemplateID,RecordID,FileName,FileType,FileSize,FileDate,FileBody,FilePath,UserName,Descript) values (?,?,?,?,?,?,?,?,?,? )";
            }
            result.close();
          }
          catch (SQLException e) {
          ...
          try {
            ...
            if (tag == 1) {
                ...
                prestmt.setString(2, mFileName);
                prestmt.setString(3, mFileType);
                prestmt.setString(4, String.valueOf(mFileSize));
                prestmt.setBytes(5, mFileBody);
                prestmt.setString(6, mFilePath);
                prestmt.setString(7, mUserName);
                prestmt.setString(8, mDescript); //"通用版本"
                ...
                prestmt.setString(1, randomId);
                prestmt.setString(2, mTemplate);
                prestmt.setString(3, mFileName);
                prestmt.setString(4, mFileType);
                prestmt.setString(5, String.valueOf(mFileSize));
                prestmt.setString(6, mFileDate);
          ...
          try {
            ...
            while (result.next()) {
              try {
                ...
                if(mBookMarkName.equals("Item")){
                	mBookMarkValue="●我省一季度盘活存量建设用地2.82万亩  "+
                					"\r\n●我省发布生物制药工业污染物排放标准填补国内空白"+
                					"\r\n●我省建立推行权力清单制度咨询顾问团队";
                }
                 if(mBookMarkName.equals("Content")){
                	mBookMarkValue="△我省一季度盘活存量建设用地2.82万亩。年初以来，全省国土资源系统积极主动服务，坚持改革创新，积极助推“三改一拆”行动，深入实施亩产倍增行动计划，全面推进空间换地和低效用地再开发，形成增量得到有效控制、存量得到有力盘活、资源利用方式和结构呈现出积极变化的良好态势。一季度全省批准建设用地12.34万亩，同比减少25.09%，供应建设用地5.71万亩，同比减少17.90%，其中存量建设用地供应面积2.82万亩，同比提高了32.65%。（省国土资源厅）"+
                					"\r\n△我省发布生物制药工业污染物排放标准填补国内空白。近日，我省正式批准发布强制性地方标准《生物制药工业污染物排放标准》，并将于2014年5月1日正式实施。我省生物制药行业约有半数企业废水通过预处理后间接排入城镇集中式污水处理厂，现行的标准体系不能满足我省生物制药行业环境监管的需求。该标准对《大气污染物综合排放标准》、《发酵类制药工业水污染排放标准》等国家标准未作规定的污染物排放指标进行了补充，填补了国内空白。（省质监局）"+
                					"\r\n△我省建立推行权力清单制度咨询顾问团队。日前，省编委办召开省推行权力清单制度咨询顾问组第一次会议，陈小恩等8位咨询顾问参加。省推行权力清单制度咨询顾问组由熟悉省市县各个层级运作、具有相当研究能力、曾经担任省政府部门领导职务、威望高的同志组成，主要是对省政府部门开展职权清理推行权力清单制度工作进行指导、把关，提高这项工作特别是审核工作的科学性、权威性。（省编委办）";
                } 
        ...
        if (DbaObj.OpenConnection()) {
          try {
            ...
            for (mIndex = 7; mIndex <= MsgObj.GetFieldCount() - 1; mIndex++) {
              ...
              Sql = "insert into Template_BookMarks (BookMarkID,RecordId,BookMarkName) values ('" + randomId + "','" + mTemplate + "','" + mBookMarkName + "')";
        ...
        if (request.getMethod().equalsIgnoreCase("POST")) {
          ...
          if (MsgObj.GetMsgByName("DBSTEP").equalsIgnoreCase("DBSTEP")) {         //判断是否是合法的信息包，或者数据包信息是否完整
            ...
            if (mOption.equalsIgnoreCase("LOADFILE")) {                           //下面的代码为打开服务器数据库里的文件
              ...
              if (MsgObj.MsgFileLoad(mFilePath+"\\ShareDoc\\"+mFileName))	{		            //从文件夹调入文档
             // if (LoadFile()) {                                                   //从数据库调入文档
              ...
              System.out.println(mFileSize+":"+mFileBody.length);
              MsgObj.MsgTextClear();                                              //清除文本信息
              if (MsgObj.MsgFileSave(mFilePath+"\\ShareDoc\\"+mFileName)){}			            //保存文档内容到文件夹中
             /* if (SaveFile()) {                                                   //保存文档内容到数据库中
              ...
              if (mCommand.equalsIgnoreCase("INSERTFILE")) {
                if (MsgObj.MsgFileLoad(mFilePath + "\\ShareDoc\\" + mTemplate)) { //从服务器文件夹中调入模板文档
              ...
              mFileBody = MsgObj.MsgFileBody();                                   //取得文档内容
              MsgObj.MsgTextClear();
              System.out.println(mFilePath+"\\ShareDoc\\"+mTemplate+mFileType);
              if (MsgObj.MsgFileSave(mFilePath+"\\ShareDoc\\"+mTemplate+mFileType)){}	
              ...
              mFileName=MsgObj.GetMsgByName("FILENAME");	                        //取得文档名称
              MsgObj.MsgTextClear();					                                    //清除文本信息
              if (MsgObj.MsgFileSave(mFilePath+"\\ShareDoc\\"+mRecordID+".pdf")){ //保存文档到文件夹中
                MsgObj.SetMsgByName("STATUS", "保存成功!");                       //设置状态信息
                MsgObj.MsgError("");					                                    //清除错误信息
              }
              else {
              ...
              mFilePath = mFilePath + "\\ShareDoc\\" + mImageName;                //图片在服务器的完整路径
              ...
              MsgObj.MsgTextClear();                                              //清除文本信息
              mFilePath = mFilePath + "\\ShareDoc\\" + mRemoteFile;
              ...
              mRecordID = MsgObj.GetMsgByName("RECORDID");                        //取得文档编号
              MsgObj.MsgTextClear();                                              //清除文本信息
              mFilePath = mFilePath + "\\ShareDoc\\" + mRecordID + ".pgf";        //全文批注文件的完整路径
