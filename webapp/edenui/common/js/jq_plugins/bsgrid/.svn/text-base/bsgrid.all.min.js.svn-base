...
//增加更改数据源-url地址方法  gridObj.changeUrl();
...
$.bsgrid = {
    ...
    a.fn.bsgrid_paging = {
        ...
        init: function(h, g) {
            ...
            var b = {
                ...
                refreshInitPage:function(){
                    a.fn.bsgrid_paging.refreshInitPage(d)
                },
                firstPage: function() {
        ...
        refreshInitPage: function(b) {
            a.fn.bsgrid_paging.page(1, b)
        },
        firstPage: function(b) {
            $(".table-fixed-header").find(".bsgrid_editgrid_check").prop("checked",false);
        ...
        createPagingToolbar: function(b) {
            ...
            if (b.settings.pageSizeSelect) {
                e.append("<td></td>");
                e.append("<td width='150'>" + a.bsgridLanguage.pagingToolbar.pageSizeDisplay(b.pageSizeId, c) + "</td>")
            }
            //e.append("<td>" + a.bsgridLanguage.pagingToolbar.currentDisplayRows(b.startRowId, b.endRowId, c) + "</td>");
            e.append("<td width='50'>" + a.bsgridLanguage.pagingToolbar.totalRows(b.totalRowsId) + "</td>");
            var d = b.settings.pagingBtnClass;
            e.append("<td width='100'>");
            ...
            e.append("</td>");
            e.append("<td width='90'>" + a.bsgridLanguage.pagingToolbar.currentDisplayPageAndTotalPages(b.curPageId, b.totalPagesId) + "</td>");
            e.append("<td width='100'>");
    ...
    $.fn.bsgrid = {
        ...
        init: function(gridId, settings) {
            ...
            var gridFather=$("#"+gridId).closest(".grid-father"),
                gridFatherHeight=gridFather.height();
            settings.pageSize=Math.floor(gridFatherHeight/38)-2;
            ...
            var gridObj = {
                ...
                getGridHeaderFixd:function(){
                    return $.fn.bsgrid.getGridHeaderFixd(options);
                },
                ...
                setHideCell:function(index){
                    return $.fn.bsgrid.setHideCell(index,options)
                },
                refreshPage: function() {
                    $.fn.bsgrid.refreshPage(options)
                },
                refreshInitPage:function(){
                    $.fn.bsgrid.refreshInitPage(options);
                },

                //选中的checked
                selectRowChecked:function(array){
                    var array=array,trLength=this.getRows(),table=trLength.closest("table");
                    table.find(".bsgrid_editgrid_check").prop("checked",false);
                    for(var i= 0,max=trLength.length;i<max;i++){
                        var idFather=trLength.eq(i).attr("id");
                        for(var j= 0,maxin=array.length;j<maxin;j++){
                            var id="grid_"+array[j];
                            if(id==idFather){
                                trLength.eq(i).find(".bsgrid_editgrid_check").prop("checked",true);
                            }
                        }
                    }
                    //头部的选择行
                    if(this.getCheckedRowsIndexs().length==trLength.length){
                        table.find("thead").find(".bsgrid_editgrid_check").prop("checked",true);
                    }
                    else{
                        table.find("thead").find(".bsgrid_editgrid_check").prop("checked",false);

                    }

                },
                //加上id，index值，去掉javascript，
                initGrid:function(){

                    //初始化加载滚动条插件


                    var head="#"+options.gridId+" thead tr th";

                    var headerFixed="#"+options.gridId+"_fixedHeader thead tr th";
                    var tableGird="#"+options.gridId;

                    var tableheaderFixed="#"+options.gridId+"_fixedHeader";
                    var trLength=this.getRows();

                    for(var i= 0,max=trLength.length;i<max;i++){
                        if(i%2===1){
                            this.getRow(i).addClass("interlaced-th");
                        }
                        this.getRow(i).attr("id","grid_" + this.getCell(i,0).html());
                        this.getRow(i).attr("index",i);
                        this.getRow(i).find("td").last().css({"border-right":0});
                        var operateHmtl=trLength.eq(i).find("td").last().html();
                        operateHmtl=operateHmtl.split("^javascript:void(0)");
                        trLength.eq(i).find("td").last().html(operateHmtl[0]);
                        //清楚木有数据的TR
                        if(this.getCell(i,0).html()=="&nbsp;"){
                            trLength.slice(i).remove();
                        }
                    }
                    $(head).last().css({"border-right":0});
                    $(headerFixed).last().css({"border-right":0});
                    ...
                    //背景显示红色
                    (function(){
                        var td="#"+options.gridId+" tbody tr td";
                        $(td).each(function(){

                            var $this=$(this),
                                $titleHtml=$this.html(),
                                $titlea =$titleHtml.indexOf("onclick");

                            if($titleHtml.indexOf("oaUrgencyTag")>(-1)){

                                $this.closest("tr").addClass("urgencytag-th");
                            }
                    ...
                    setTimeout(function(){
                        ...
                        $(td).each(function(){
                            ...
                            var $this=$(this),
                                $titleHtml=$this.html(),
                                $titlea =$titleHtml.indexOf("onclick"),
                                $titleinput=$titleHtml.indexOf("checkbox"),
                                $titleClassTitle=$this.find(".oaOnclickTitle"),
                                $titleGridLeader=$this.find(".grid-leader"),
                                $title;



                            if($titlea>0||$titleinput>0){
                                $title="";
                            }
                            else{
                                $title=$titleHtml;
                            }
                            //如果有oaOnclickTitle标志位的
                            if($titleClassTitle.length){
                                $title=$titleClassTitle.html();
                            }
                            //领导批示的标志位
                            if($titleGridLeader.length){
                                $this.closest("tr").addClass("td-grid-leader");
                            }
                            $this.attr("title",clearNbsp($title));
                        });
                    ...
                    //checkbox 居中显示
                    var checked=$(".bsgrid_editgrid_check");

                    if(checked.length){
                        checked.each(function(){
                            var isParentTd=$(this).parent()[0].tagName;
                            if(isParentTd==="td"){
                                $(this).closest("td").css({"textAlign": "center","paddingLeft":"0px"});
                            }
                    ...
                    (function () {
                        var openerHref = window.location.href;
                        //不加载此方法的页面
                        var unLoadData = [];
                        var isgoing = true;
                       for(var x=0;x<unLoadData.length;x++){

                           if(openerHref.indexOf(unLoadData[x])>(-1)){
                        ...
                        if (isgoing) {
                        var tr = "#" + options.gridId + " tbody tr";
                            //加载此方法的页面，存在多个一样的a 标签方法
                            //公文管理:所有页面
                            //会议:待办会议，在办会议，已收会议，会议报名，所有会议查看，历史会议查看
                            // 领导批示:待办批示，在办批示，办结批示，所有批示查看
                        var includeHtml=["waitDocument/waitDocument.action","waitDocument/backDocument.action","incoming/incomingManage.action?wfType=noDo&fileType=","incoming/incomingManage.action?wfType=hasDo&fileType=","documentPassRead/noReadManage.action?rtype=oa_incoming_type","incoming/incomingManage.action?wfType=hasEnd&fileType=","incoming/incomingManage.action?wfType=","sendDocument/sendDocumentAction!sendDocument.action?wfType=","sendDocument/sendDocumentAction!sendDocument.action?wfType=","incomingHis/incomingManage.action?wfType=incomingUnit","sendDocumentHis/sendDocumentAction!sendDocument.action?wfType=sendDocumentDepartment","meeting/meetingInfoManage.action?wfType=noDo","meeting/meetingInfoManage.action?wfType=hasDo","meeting/meetingInfoManage.action?wfType=received","hallRegist/hallRegistManage.action","meeting/meetingInfoManage.action?queryType=","meetingHistory/meetingInfoHistoryManage.action?queryType=meetingType","instruction/executeInst.action?iType=toDo","instruction/executeInst.action?iType=doing","instruction/executeInst.action?iType=done","instruction/executeAllInst.action?allType="],
                            isInclude=false;

                            for(var i=0,max=includeHtml.length;i<max;i++){
                                if(openerHref.indexOf(includeHtml[i])>(-1)){
                                    isInclude=true;
                                    break;
                                }
                            }
                        ...
                        $(tr).each(function () {
                            var $this = $(this),
                                $aOnclick = $this.find("a"),
                                $alength=$this.find("a").length;

                            if ($alength == "1"||isInclude) {

                                //tr增加onclick事件
                                $this.attr("onclick", $aOnclick.eq(0).attr("onclick"));
                                //取消a 标签自带的click事件
                                $aOnclick.removeAttr("onclick").css({"color": "#333"});
                                $this.find("td").css({"cursor": "pointer"});

                               var  redLine=$this.find(".oaUrgencyTag");
                                if(redLine.length){
                                    $this.find("a").css({"color":"#ff2400"});
                                }
                ...
                //隐藏单列
                setHideCell:function(index){
                    var head="#"+options.gridId+" thead tr:last th";
                    var headFixed="#"+options.gridId+"_fixedHeader thead tr th"
                    $(head).eq(index).hide();
                    $(headFixed).eq(index).hide();
                    this.getColCells(index).hide();
                },
                //显示单列
                setShowCell:function(index){
                    var head="#"+options.gridId+" thead tr:last th";
                    var headFixed="#"+options.gridId+"_fixedHeader thead tr th"
                    $(head).eq(index).show();
                    $(headFixed).eq(index).show();
                    this.getColCells(index).show();
                },
                //替换规则 index:td值从1开始，obj:替换规则
                changeCell:function(index,obj){
                    var index=index,obj=obj;
                    var getCell=this.getColCells(index);
                    for(var j= 0,max=getCell.length;j<max;j++){
                        var content=getCell.eq(j).html();
                        getCell.eq(j).html(change(content));
                    }

                    function change(content){
                        var name=content,result="";
                ...
                changeUrl:function(url){
                    ...
                    if(url){
                        options.settings.url=url;
                        this.refreshInitPage();
                    }
        ...
        loadGridData: function(dataType, gridData, options) {
            ...
            if (parseSuccess) {
                ...
                options.page.curPage=curPage;

                // options.curPage = curPage;
                options.curPage = options.page.curPage;
        ...
        sort: function(obj, options) {
            ...
            //改变自定义表头样式
            $.fn.bsgrid.getGridHeaderFixd(options).each(function(i) {
                var sortName = columnsModel[i].sortName;
                if (sortName != "") {
                    var sortOrder = $.fn.bsgrid.getSortOrder($(this), options);
                    if (!options.settings.multiSort && sortName != field) {
        ...
        getGridHeaderFixd:function(options){
            return $("#" + options.gridId + "_fixedHeader thead tr").find("th");
        },
        ...
        setGridBlankBody: function(options) {
            ...
            if (options.settings.pageSize > 0) {
                ...
                trSb.append("<tr>");
                for (var hi = 0; hi < header.length; hi++) {
                    trSb.append('<td  style="text-align1: ' + columnsModel[hi].align + ";");
                    if (columnsModel[hi].hidden == "true") {
            ...
            if (curPageRowsNum == 0) {
                //rowsSb.append('<tr><td style="text-align:center!important;" colspan="' + header.length + '">' + $.bsgridLanguage.noDataToDisplay + "</td></tr>")

                rowsSb.append('<tr><td  style="text-align:center!important;width:auto;" colspan="' + (header.length-1) + '">' + $.bsgridLanguage.noDataToDisplay + "</td></tr>")
            } else {
        ...
        addLockScreen: function(options) {
            if ($(".bsgrid.lockscreen").length == 0) {
                ...
                //$("#common_table .ver-top").append(lockScreenHtml.toString());
            }
        },
        lockScreen: function(options) {
            ...
            if ($(".bsgrid.lockscreen").css("display") == "none") {
                ...
                //计算遮罩层的宽度
                var lockscreenWidth=$("#common_table .ver-top").width();

                $(".bsgrid.lockscreen").width(lockscreenWidth);
                $(".bsgrid.loading_div").width(lockscreenWidth);
            }
        },
        ...
        refreshInitPage:function(options) {
            if (!options.settings.pageAll) {
                $.fn.bsgrid.getPagingObj(options).refreshInitPage()
            } else {
                $.fn.bsgrid.page(1, options)
            }
        },
