  ...
  $(document).on("blur",'textarea, input, select',function(){
		...
		if(e.attr("data-validate")){
			...
			if($checkstate){
				...
				//追加提示在底部
				var nowGroup=e.closest(".form-group");		
				var totalGroup;
				
				if(nowGroup.hasClass("x6")){
					
					if(nowGroup.hasClass("form-group-right")){
						totalGroup=nowGroup.prev(".form-group");
							
					}
					else{
						totalGroup=nowGroup.next(".form-group");
					}
					
					if(totalGroup.find(".input-help")){
						totalGroup.find(".input-help").remove();
					}
				...
				if(nowGroup.hasClass("x6")){
					...
					if(totalGroup.find('.input-help').length<1){
						totalGroup.find('.field').append('<div class="input-help"><ul><li>&nbsp;</li></ul></div>');
					}
	...
	$pintuercheck=function(element,type,value){
		...
		switch(type){
			...
			case "mobile":return  /^\d{8,11}$/.test($pintu);break;
...
function pintuerFormCheck(id){

  var $this=$(id).closest("form");
  $this.find('input[data-validate],textarea[data-validate],select[data-validate]').trigger("blur");
  var numError = $this.find('.check-error').length;
  if(numError){
    $this.find('.check-error').first().find('input[data-validate],textarea[data-validate],select[data-validate]').first().focus().select();
